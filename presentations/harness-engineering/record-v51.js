(() => {
  ...
  soundBtn.addEventListener('click',()=>{
    ...
    soundEnabled=!soundEnabled;syncSound();
  });
  ...
  function startTimer(){
    ...
    timerId=setInterval(()=>{
      ...
      document.title=`● REC ${t} · Project6 v51`;
    },250);
  }
  ...
  function resetUI(doneMs=0){
    ...
    stopBtn.disabled=true;openBtn.disabled=false;soundBtn.disabled=false;
    stageFrame.classList.remove('recording');guide.textContent='OUTPUT · 1920×1080';
  }
  ...
  async function finish(ext,mime){
    ...
    a.href=url;a.download=`project6-harness-v51-1080p-${new Date().toISOString().replace(/[:.]/g,'-')}.${ext}`;
    document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),5000);
    stateEl.textContent=`DONE · ${ext.toUpperCase()} · ${(blob.size/1024/1024).toFixed(1)}MB`;
    document.title=`Project6 v51 · DONE ${clock(elapsed)}`;
  ...
  async function start(){
    ...
    rawStream=await navigator.mediaDevices.getDisplayMedia({video:true,audio:soundEnabled,preferCurrentTab:true,selfBrowserSurface:'include',surfaceSwitching:'exclude'});
    ...
    if(soundEnabled)rawStream.getAudioTracks().forEach(t=>composedStream.addTrack(t));
    ...
    openBtn.disabled=true;stopBtn.disabled=false;soundBtn.disabled=true;
    stageFrame.classList.add('recording');guide.textContent='● RECORDING · 1920×1080';
  ...
  function openOverlay(){
    ...
    info.textContent='READY · 最终只录中间 16:9 成片框 · 1920×1080 OUTPUT';
  }
