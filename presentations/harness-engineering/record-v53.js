(() => {
  ...
  function syncTransportLabel(){
    ...
    if(s==='PAUSED')playState.textContent='已暂停';
    else if(s==='END')playState.textContent='已结束';
    else if(s.includes('PLAY')||s.includes('RUN'))playState.textContent='自动播放中';
    else playState.textContent='播放器直连';
  }
  if(status){new MutationObserver(syncTransportLabel).observe(status,{childList:true,characterData:true,subtree:true});}
  [prevBtn,nextBtn,playBtn,pauseBtn].forEach(btn=>btn?.addEventListener('click',()=>setTimeout(syncTransportLabel,30)));
  ...
  musicFile.addEventListener('change',()=>{
    ...
    bgm.addEventListener('loadedmetadata',()=>{
      if(Number.isFinite(bgm.duration)) musicName.textContent=`${file.name} · ${clock(bgm.duration*1000)}`;
    },{once:true});
  });
  musicToggle.addEventListener('click',async()=>{
    ...
    if(musicWanted){
      try{await bgm.play();musicToggle.textContent='⏸ 配乐';musicToggle.classList.add('on')}
      catch(err){musicWanted=false;musicToggle.textContent='▶ 配乐';musicToggle.classList.remove('on')}
    }else{
      bgm.pause();musicToggle.textContent='▶ 配乐';musicToggle.classList.remove('on');
    }
  });
  ...
  function startTimer(){
    ...
    timerId=setInterval(()=>{const t=clock(Date.now()-startedAt);timerEl.innerHTML=`<span class="rec-dot"></span>REC ${t}`;document.title=`● REC ${t} · Project6 v53`},250);
  }
  ...
  async function stop(reason='manual'){
    if(stopping)return;stopping=true;pauseBtn?.click();observer?.disconnect();observer=null;
  ...
  async function finish(ext,mime){
    ...
    const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`project6-harness-v53-1080p-${new Date().toISOString().replace(/[:.]/g,'-')}.${ext}`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),5000);
    stateEl.textContent=`DONE · ${ext.toUpperCase()} · ${(blob.size/1024/1024).toFixed(1)}MB`;document.title=`Project6 v53 · DONE ${clock(elapsed)}`;resetUI(elapsed);stopping=false;recorder=null;rawStream=null;composedStream=null;
  }
  ...
  async function start(){
    ...
    overlay.classList.add('hidden');await resetPlayer();drawFrame();
    ...
    openBtn.disabled=true;stopBtn.disabled=false;stageFrame.classList.add('recording');stateEl.textContent=`LIVE · DIRECT CONTROLS · ${source.videoWidth}×${source.videoHeight} SOURCE`;
    playBtn?.click();
  ...
  function openOverlay(){if(recorder?.state==='recording')return;overlay.classList.remove('hidden');info.textContent='READY · 真实播放器按钮已直连 · 最终只录中间 16:9 成片框';}
