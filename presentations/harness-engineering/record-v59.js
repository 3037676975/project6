(() => {
  ...
  function openCaptureSurface(){
    const url=new URL('./capture.html?v=59',location.href);
    captureWin=window.open(url.href,'project6Capture','popup=yes,width=1920,height=1080,resizable=yes');
    if(!captureWin){alert('浏览器阻止了成片窗口，请允许本站弹出窗口。');return}
    captureState.textContent='正在连接…';
  }
  ...
  channel.onmessage=e=>{
    const m=e.data||{};
    if(m.type==='capture-ready'){captureReady=true;captureState.textContent='已连接 · PURE 16:9';}
    if(m.type==='state'){
      captureReady=true;captureState.textContent='已连接 · PURE 16:9';
      playerState.textContent=`画面状态：${m.status||'READY'} · ${m.count||'01 / 42'}`;
    }
  };
  ...
  musicFile.addEventListener('change',()=>{
    const f=musicFile.files?.[0]; if(!f)return; musicSelected=f; musicWanted=false; musicToggle.disabled=false; musicToggle.textContent='▶ 配乐'; musicName.textContent=f.name;
    channel.postMessage({type:'music-file',file:f,volume:Number(musicVolume.value)/100,play:false});
  });
  ...
  async function begin(){
    ...
    if(!captureReady)throw new Error('请先打开纯成片窗口。');
    const p=preset(); quality.disabled=true; recState.textContent='请选择“Project6 v59 · Harness Capture Surface”标签页';
    rawStream=await navigator.mediaDevices.getDisplayMedia({video:{width:{ideal:p.w},height:{ideal:p.h},frameRate:{ideal:60,max:60}},audio:true,surfaceSwitching:'exclude'});
    const vt=rawStream.getVideoTracks()[0]; if(!vt)throw new Error('没有获得视频轨道。'); vt.addEventListener('ended',()=>finalize(true),{once:true});
    ...
    canvas.width=p.w;canvas.height=p.h;draw(); outStream=canvas.captureStream(60);rawStream.getAudioTracks().forEach(t=>outStream.addTrack(t));
    const c=chooseMime(),opts=c.mime?{mimeType:c.mime,videoBitsPerSecond:p.bps,audioBitsPerSecond:256_000}:{videoBitsPerSecond:p.bps};chunks=[];cancelled=false;pausedAt=0;totalPaused=0;recorder=new MediaRecorder(outStream,opts);recorder.addEventListener('dataavailable',e=>{if(e.data?.size)chunks.push(e.data)});recorder.addEventListener('stop',()=>finish(c.ext,c.mime||recorder.mimeType),{once:true});recorder.start();startedAt=Date.now();startTimer();
    start.disabled=true;pauseRec.disabled=false;stop.disabled=false;cancel.disabled=false;
    const lower=source.videoWidth<p.w||source.videoHeight<p.h;sourceInfo.textContent=`SOURCE ${source.videoWidth}×${source.videoHeight} → OUTPUT ${p.w}×${p.h}${lower?' · 源分辨率低于输出，存在放大':''}`;recState.textContent=`LIVE · ${p.label} · 纯成片窗口捕获`;
  }
