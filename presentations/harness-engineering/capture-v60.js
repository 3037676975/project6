(() => {
  ...
  const prev=$('prevBtn'),next=$('nextBtn'),pause=$('pauseBtn'),bgm=$('bgm'),count=$('countLabel'),status=$('status');
  let musicUrl='';
  ...
  channel.onmessage=async e=>{
    ...
    if(m.type==='voice-source'){window.P6VoiceLibrary?.setActive?.(m.id||'');window.dispatchEvent(new CustomEvent('p6-voice-source-change',{detail:{id:m.id||''}}));sendState()}
