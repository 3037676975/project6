(() => {
  ...
  function mountAudioUnlock(){
    ...
    const wrap=document.createElement('div');wrap.id='p6AudioUnlock';wrap.innerHTML='<button type="button">🔊 启用旁白声音</button><small>录制前只需点一次，随后自动隐藏</small>';
    ...
    const btn=wrap.querySelector('button');Object.assign(btn.style,{border:'0',borderRadius:'11px',padding:'11px 16px',background:'#13b89f',color:'#062822',fontWeight:'900',cursor:'pointer'});
    wrap.querySelector('small').style.opacity='.72';
    btn.addEventListener('click',async()=>{
      sound.muted=false;
      window.dispatchEvent(new CustomEvent('p6-voice-source-change',{detail:{id:window.P6VoiceLibrary?.active?.()||''}}));
      await new Promise(r=>setTimeout(r,100));
      if(sound.src){const old=sound.volume;sound.volume=0.001;try{await sound.play();sound.pause();sound.currentTime=0}catch(_){}sound.volume=old;}
      sessionStorage.setItem('project6.harness.audio.unlocked','1');wrap.remove();channel.postMessage({type:'audio-unlocked',ok:true});
    });
  ...
  channel.onmessage=async e=>{
    const m=e.data||{};
    if(m.type==='cmd'){
      if(m.action==='prev')prev?.click();if(m.action==='next')next?.click();if(m.action==='play')play?.click();if(m.action==='pause')pause?.click();if(m.action==='sound'&&sound)sound.muted=!m.enabled;
  ...
  addEventListener('load',async()=>{if(document.fonts?.ready)await document.fonts.ready;if(sessionStorage.getItem('project6.harness.audio.unlocked')!=='1')mountAudioUnlock();setTimeout(()=>{sendState();channel.postMessage({type:'capture-ready'});},160)});
