(() => {
  ...
  let rawStream=null,outStream=null,recorder=null,chunks=[],raf=0,startedAt=0,timerId=0,pausedAt=0,totalPaused=0,cancelled=false,stopping=false,mixCtx=null;
  ...
  function openCaptureSurface(){const url=new URL('./capture.html?v=62',location.href);captureWin=window.open(url.href,'project6Capture','popup=yes,width=1920,height=1080,resizable=yes');if(!captureWin){alert('浏览器阻止了成片窗口，请允许本站弹出窗口。');return}captureState.textContent='正在连接…'}
  ...
  sound.addEventListener('click',()=>{soundEnabled=!soundEnabled;sound.textContent=soundEnabled?'🔊 旁白开':'🔇 旁白关';sound.classList.toggle('primary',soundEnabled)});
  ...
  function stopTracks(){try{rawStream?.getTracks().forEach(t=>t.stop())}catch(_){}try{outStream?.getTracks().forEach(t=>t.stop())}catch(_){}try{mixCtx?.close()}catch(_){}mixCtx=null}
  ...
  async function attachMixedAudio(){
    const rawAudio=rawStream?.getAudioTracks?.()||[];
    const voiceStream=window.P6_VOICE_MIX_STREAM;
    const voiceAudio=voiceStream?.getAudioTracks?.()||[];
    if(!rawAudio.length&&!voiceAudio.length)return;
    mixCtx=new (window.AudioContext||window.webkitAudioContext)();await mixCtx.resume();const dest=mixCtx.createMediaStreamDestination();
    if(rawAudio.length){mixCtx.createMediaStreamSource(new MediaStream(rawAudio)).connect(dest)}
    if(voiceAudio.length){mixCtx.createMediaStreamSource(voiceStream).connect(dest)}
  ...
  async function begin(){if(!window.isSecureContext||!navigator.mediaDevices?.getDisplayMedia)throw new Error('请使用 HTTPS + 最新版 Chrome / Edge。');if(!captureReady)throw new Error('请先打开纯成片窗口。');const p=preset();quality.disabled=true;recState.textContent='请选择“Project6 v62 · Harness Capture Surface”标签页';rawStream=await navigator.mediaDevices.getDisplayMedia({video:{width:{ideal:p.w},height:{ideal:p.h},frameRate:{ideal:60,max:60}},audio:true,surfaceSwitching:'exclude'});const vt=rawStream.getVideoTracks()[0];if(!vt)throw new Error('没有获得视频轨道。');vt.addEventListener('ended',()=>finalize(true),{once:true});source.srcObject=rawStream;await source.play();await new Promise(r=>source.readyState>=2&&source.videoWidth?r():source.addEventListener('loadedmetadata',r,{once:true}));canvas.width=p.w;canvas.height=p.h;draw();outStream=canvas.captureStream(60);await attachMixedAudio();const c=chooseMime(),opts=c.mime?{mimeType:c.mime,videoBitsPerSecond:p.bps,audioBitsPerSecond:256_000}:{videoBitsPerSecond:p.bps};chunks=[];cancelled=false;pausedAt=0;totalPaused=0;recorder=new MediaRecorder(outStream,opts);recorder.addEventListener('dataavailable',e=>{if(e.data?.size)chunks.push(e.data)});recorder.addEventListener('stop',()=>finish(c.ext,c.mime||recorder.mimeType),{once:true});recorder.start();startedAt=Date.now();startTimer();start.disabled=true;pauseRec.disabled=false;stop.disabled=false;cancel.disabled=false;const lower=source.videoWidth<p.w||source.videoHeight<p.h;sourceInfo.textContent=`SOURCE ${source.videoWidth}×${source.videoHeight} → OUTPUT ${p.w}×${p.h}${lower?' · 源分辨率低于输出，存在放大':''}`;recState.textContent=`LIVE · ${p.label} · TTS 默认混音`}
