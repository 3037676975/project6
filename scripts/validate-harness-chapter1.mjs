...
check('ambient motion is semantic',js.includes('startAmbient')&&js.includes('motionPath')&&js.includes('runtime-card .mini i')&&js.includes("row:not(.head) .bar i"));
...
check('acceptance v9',acceptance.includes('第一章验收标准 v9')&&acceptance.includes('Cloud / SaaS Design')&&acceptance.includes('不使用 Emoji'));

const pass=checks.every(c=>c.ok);
console.log('\nHarness Chapter 1 Gate v9 — Garden + Cloud hierarchy + Emil + GSAP\n');
for(const c of checks)console.log(`${c.ok?'PASS':'FAIL'}  ${c.name}${c.detail?` · ${c.detail}`:''}`);
console.log(`\nCODE GATE: ${pass?'PASS':'FAIL'}`);console.log('USER VISUAL REVIEW: REQUIRED');
