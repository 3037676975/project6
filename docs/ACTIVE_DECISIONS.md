...
## 2026-09-11 · Project 002 必须复制 Project 001 的完整生产框架
...
- 但必须把它当作 **完整项目结构** 的基线，而不只是视觉参考。

### 2. Project 002 必须具备的完整结构

第二个视频项目不是 4 张预览图 Demo。必须像 Project 001 一样完整包含：
...
播放器控制也必须对齐 Project 001：上一张、下一张、播放、暂停、声音、全屏、高清录制、Scene 进度。
...
当前为 9 Chapters / 42 Scenes。

### 4. Project 002 视觉硬规则

- 禁止再使用 CSS 绘制的黑色小黑云 / 黑色云朵 mascot；
- 项目识别只保留左上角文字 HUD；
- 1920×1080；
- warm-keynote 米白网格；
- 暖棕正文 + 青绿强调；
- 画面信息量必须和口播信息量匹配；
- 一句口播含多个事实时，必须用时间线、对比表、流程、业务矩阵等把事实可视化；
- 每个 Scene 解释关系 / 空间 / 状态 / 过程；
- Scene 001～004 是当前 Visual Anchor，确认后继续 Scene 005～042。

### 5. Gate 与“框架存在”的区别

完整框架可以提前搭好，包括 42 段 TTS manifest、导入器、播放器、Recorder / Capture；但不能把“框架已经存在”写成 Gate 已 PASS。

当前原则：

```text
42 Scene 口播 REVIEW
→ 001～004 Visual Anchor REVIEW
→ 42 段 TTS manifest / 导入框架 REVIEW READY
→ 用户确认后继续 005～042 Garden
→ 全片 Screenshot QA
→ 实际生成并导入 IndexTTS
→ 整片音画 PASS
→ SFX / BGM
...
本轮 Project 002 仍在 Gate REVIEW，不升级全局 canonical，使用项目级缓存后缀 `64-p2a5`，避免为了第二支视频强制改动 Project 001 已完成入口。
